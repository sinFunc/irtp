...
int SendDataRtpSession(CRtpSessionManager* p,const uint8_t* buf,int len,uint16_t marker,int pt=-1);
int RcvDataRtpSession(CRtpSessionManager* p,uint8_t* buf,int len,CRcvCb rcvCb,void* user);
int SendDataWithTsRtpSession(CRtpSessionManager* p,const uint8_t* buf,int len,uint32_t pts,uint16_t marker,int pt=-1);
...
CRtpSessionInitData*  CreateRtpSessionInitData(const char* localIp,const char* remoteIp,int localPort
                                               ,int remotePort,int payloadType,int clockRate,int fps=25);
